...
class vector(object):
    ...
    def random():
        return vector(
            -1.0 + 2.0 * random(), -1.0 + 2.0 * random(), -1.0 + 2.0 * random()
        )
    ...
    def __truediv__(
        self, other
    ):  # used by Python 3, and by Python 2 in the presence of __future__ division
        try:
...
def adjust_up(oldaxis, newaxis, up, save_oldaxis):  # adjust up when axis is changed
    if abs(newaxis.x) + abs(newaxis.y) + abs(newaxis.z) == 0:
        ...
        if save_oldaxis is None:
            save_oldaxis = oldaxis
        return save_oldaxis
